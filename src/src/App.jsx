...
export default function App(){
  ...
  const filesToBase64 = (files, done) => {
...
Firebase Storage 규칙과 Storage 생성 여부를 확인해 주세요.`);
  ...
  const quoteText=()=>[`성함 : ${form.name||"미입력"}`,`업체명 : ${form.company||"미입력"}`,`연락처 : ${form.phone||"미입력"}`,`이메일 : ${email()||"미입력"}`,`행사 장소 : ${form.location||"미입력"}`,`행사일 : ${form.eventDate||"미입력"}`,"","[렌탈 요청 품목]",...(selectedItems.length?selectedItems.map(i=>`- ${i.categoryName} / ${i.name} : ${qty[i.id]}개`):["선택한 품목 없음"]),"","[추가 요청사항]",form.notes||"없음"].join("\n");
  const send=()=>{for(const f of [["성함",form.name],["업체명",form.company],["연락처",form.phone],["이메일",email()],["비밀번호",form.password],["행사 장소",form.location],["행사일",form.eventDate]]) if(!String(f[1]||"").trim()) return setMsg({type:"warn",text:`${f[0]}을(를) 입력해 주세요.`}); if(!selectedItems.length) return setMsg({type:"warn",text:"렌탈 품목을 1개 이상 선택해 주세요."}); const q=quoteText(), ni={id:Date.now(),createdAt:new Date().toLocaleString("ko-KR"),name:form.name,company:form.company,phone:form.phone,email:email(),password:form.password,location:form.location,eventDate:form.eventDate,items:selectedItems.map(i=>({categoryName:i.categoryName,name:i.name,quantity:qty[i.id]}))}; setInquiries(p=>[ni,...p]);setSelId(ni.id);setForm(emptyForm);setQty(Object.fromEntries(allItems.map(i=>[i.id,""])));setMsg({type:"success",text:"성공적으로 견적 문의가 접수되었습니다."});};
